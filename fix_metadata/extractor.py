...
def extract_metadata_from_azw3(file_path):
                ...
                try:
                    ...
                    if os.path.exists(opf_filename):
                        ...
                        with open(opf_filename, 'rb') as f_opf:
                            ...
                            g, p, t, s, sub = extract_metadata_from_opf(opf_content)
    ...
    logger.info(f"Subtítulo: {'✓ ' + subtitle if subtitle else '✗ No encontrado'}")
    logger.info(f"Temas: {len(subjects)} encontrados")
    logger.info("=" * 70)

    return (generator, producer, title_opf, subjects, subtitle)
...
def extract_metadata_from_opf(opf_content):
    ...
    subtitle = extract_subtitle_from_opf(opf_content)
    ...
    return (generator, book_producer, title_opf, subjects, subtitle)
...
def extract_subtitle_from_opf(opf_content):
    """
    Extrae el subtítulo real del OPF.

    Soporta:
      1. calibre:subtitle  – como <meta property="calibre:subtitle">…</meta>
         o como <meta name="calibre:subtitle" content="…"/>.
      2. EPUB3 refines      – un <dc:title id="X">Subtítulo</dc:title> referido
         por <meta refines="#X" property="title-type">subtitle</meta>.
    Devuelve el subtítulo (str) o None.
    """
    if isinstance(opf_content, bytes):
        opf_content = opf_content.decode('utf-8', errors='ignore')

    logger.debug("--- Búsqueda de subtítulo en el OPF ---")

    # 1. calibre:subtitle (varias formas)
    calibre_patterns = [
        r'<meta[^>]*property=["\']calibre:subtitle["\'][^>]*>([^<]+)</meta>',
        r'<meta[^>]*name=["\']calibre:subtitle["\'][^>]*content=["\']([^"\']+)["\']',
        r'<meta[^>]*content=["\']([^"\']+)["\'][^>]*name=["\']calibre:subtitle["\']',
    ]
    for p in calibre_patterns:
        m = re.search(p, opf_content, re.IGNORECASE | re.DOTALL)
        if m and m.group(1).strip():
            result = html.unescape(m.group(1).strip())
            logger.info(f"  ✓ Subtítulo (calibre:subtitle): {result}")
            return result

    # 2. EPUB3 refines: localizar los <dc:title> con id y ver cuál es 'subtitle'
    try:
        titles_by_id = {}
        for m in re.finditer(
                r'<dc:title[^>]*\bid=["\']([^"\']+)["\'][^>]*>([^<]+)</dc:title>',
                opf_content, re.IGNORECASE | re.DOTALL):
            titles_by_id[m.group(1)] = m.group(2)

        if titles_by_id:
            # Buscar cualquier <meta …>subtitle</meta> que refine un id de título
            for m in re.finditer(r'<meta\b[^>]*>\s*subtitle\s*</meta>',
                                  opf_content, re.IGNORECASE):
                tag = m.group(0)
                if 'title-type' not in tag.lower():
                    continue
                ref = re.search(r'refines=["\']#([^"\']+)["\']', tag, re.IGNORECASE)
                if ref and ref.group(1) in titles_by_id:
                    result = html.unescape(titles_by_id[ref.group(1)].strip())
                    if result:
                        logger.info(f"  ✓ Subtítulo (EPUB3 refines): {result}")
                        return result
    except Exception as e:
        logger.debug(f"  ✗ Error buscando subtítulo por refines: {e}")

    logger.debug("  ✗ Subtítulo no encontrado en el OPF")
    return None